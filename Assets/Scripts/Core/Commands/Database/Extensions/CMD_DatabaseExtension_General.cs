...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_General : CMD_DatabaseExtension
    {
        ...
        new public static void Extend(CommandDatabase database)
        {
            ...
            database.AddCommand("endgame", new Action(PlayEnding));
            database.AddCommand("warning", new Action(PlayWarning));
            ...
            database.AddCommand("setdbpast", new Action(SetDialogueTextTransform_Past));
            database.AddCommand("setdbdefault", new Action(SetDialogueTextTransform_Default));
            
            database.AddCommand("load", new Action<string[]>(LoadNewDialogueFile));
            database.AddCommand("loadscene",new Action<string>(LoadSceneByBuildIndex));
            database.AddCommand("control", new Action(setUserControl));
        }

        private static void PlayEnding()
        {
            VNManager.instance.Ending.SetActive(true);
        }
        private static void PlayWarning()
        {
            VNManager.instance.Warning.SetActive(true);
        }
        private static void setUserControl()
        {
            if (PlayerInputManager.isCutScenePlaying)
            {
                PlayerInputManager.isCutScenePlaying = false;
            }
            else
            {
                PlayerInputManager.isCutScenePlaying = true;
            }
            
        }
        private static void LoadSceneByBuildIndex(string sceneName)
        {
            if(VNGameSave.isLoad)
            {
                VNGameSave.isLoad = false;
                
            }
            else
            {
                var save = VNGameSave.activeFile;
                save.Save();
                VNGameSave.activeFile = save;
                VNGameSave.isLoad = true;
        ...
        private static void SetDialogueTextTransform_Past()
        {
            DialogueSystem.instance.dialogueContainer.SetDialogueTextTransform_Past();
        }
        
        private static void SetDialogueTextTransform_Default()
        {
            DialogueSystem.instance.dialogueContainer.SetDialogueTextTransform_Default();
        }
